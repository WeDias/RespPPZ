# Wesley Dias (1º Semestre ADS-B), Lista IX
...
def middle_way(a, b):
    return [a[(len(a)-1)//2], b[(int(len(b)-1)//2)]]
...
def date_fashion(eu, par):
    resposta = 0
    if (eu > 2 and par >= 8) or (par > 2 and eu >= 8):
        resposta = 2
    elif (8 > eu > 2 and 2 < par < 8) or (8 > par > 2 and 2 < eu < 8):
        resposta = 1
    return resposta
...
def squirrel_play(temp, is_summer):
    resultado = False
    if (90 >= temp >= 60 and not is_summer) or (100 >= temp >= 60 and is_summer):
        resultado = True
    return resultado
