# Wesley Dias (1º Semestre ADS-B), Lista VIII
...
# Exercício A
preposicoes = 0
for palavras in txtB:
    if palavras[0] in 'zmb' and palavras[len(palavras)-1] not in 'zmb':
        preposicoes += 1
print(f'(A) Há {preposicoes} preposições no TextoB!')

# Exercício B
verbos = 0
for palavras in txtB:
    if len(palavras) == 7 and palavras[len(palavras)-1] not in 'zmb':
        verbos += 1
print(f'(B) Há {verbos} verbos no TextoB!')

# Exercício C
verbo1 = 0
for palavras in txtB:
    if len(palavras) == 7 and palavras[len(palavras)-1] not in 'zmb':
        if palavras[0] not in 'zmb':
            verbo1 += 1
print(f'(C) Há {verbo1} verbos na primeira pessoa no TextoB!')

# Exercício D
alfaN = 'abcdefghijklmnopqrst'
alfaG = 'zmbtshjpnwlrcxkqvdgf'
ordemI = []
for palavra in txtB:
    palavranova = ''
    for l in range(len(palavra)):
        palavranova += alfaN[alfaG.index(palavra[l])]
    ordemI.append(palavranova)
ordemI.sort()
ordemF = []
for palavra in ordemI:
    palavraGooglon = ''
    for l in range(len(palavra)):
        palavraGooglon += alfaG[alfaN.index(palavra[l])]
    ordemF.append(palavraGooglon)
print(f'(D) {ordemF}')
...
for palavra in txtB:
    ...
    for i, letra in enumerate(palavra):
        numero += numeros[alfaG.index(letra)] * 20 ** i
    if numero % 42 == 0:
        falso = 0
        for digitos in alfaG:
            if str(palavra).count(str(digitos)) > 1:
                falso += 1
        if falso == 0:
            cont += 1
            magico.append(palavra)
print(f'(E) São {cont} números mágicos: {magico} no TextoB!')
