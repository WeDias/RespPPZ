# Wesley Dias (1º Semestre ADS-B), Lista X
...
def count_code(s):
    ...
    for c in range(65, 91):
        res += s.count(f'co{chr(c).lower()}e')
    return res
...
def soma_na_lista(n, lista):
    ...
    for contador in range(len(lista)-1):
        for num in lista:
            if n == num + lista[len(lista)-contador-1] and num != lista[len(lista)-contador-1]:
                situ = True
    return situ
...
def fila_tijolos(n_peq, n_gra, meta):
    situ = False
    if (meta % 5 == 0 and meta // 5 <= n_gra) or (meta <= n_peq):
        situ = True
    elif (0 < (meta - n_gra * 5) <= n_peq < meta) or (meta - (n_gra // 2) * 5) == n_peq:
        situ = True
    return situ
